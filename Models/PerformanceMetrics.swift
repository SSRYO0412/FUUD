...
struct PerformanceMetrics: Codable {
    ...
    let predictedCalories: Int     // 予測消費カロリー [DUMMY] 予測計算値に置き換え
    ...
    static var sample: PerformanceMetrics {
        PerformanceMetrics(
            ...
            predictedCalories: 2450 // [DUMMY] 仮の予測消費カロリー
        )
    }
}
...
struct PerformanceDetailData {
    ...
    static var sample: PerformanceDetailData {
        PerformanceDetailData(
            ...
            predictedCalories: [
                "予測消費": "2,450kcal",  // [DUMMY] AI予測値に置き換え
                "基礎代謝": "1,850kcal",  // [DUMMY] 計算値に置き換え
                "活動代謝": "600kcal",    // [DUMMY] 計算値に置き換え
                "前日比": "+8%"           // [DUMMY] 比較値に置き換え
            ]
        )
    }
}
...
struct TuuningIntelligence {
    // [DUMMY] デフォルトメッセージ - メトリック未選択時に表示、本番ではAI総合分析結果に置き換え
    ...
    // [DUMMY] Recovery専用メッセージ - 本番ではRecovery指標分析結果に置き換え
    ...
    // [DUMMY] Metabolic専用メッセージ - 本番では代謝力分析結果に置き換え
    ...
    // [DUMMY] Inflammation専用メッセージ - 本番では炎症指標分析結果に置き換え
    ...
    // [DUMMY] Longevity（Aging pace）専用メッセージ - 本番では老化速度分析結果に置き換え
    ...
    // [DUMMY] Performance専用メッセージ - 本番では総合パフォーマンス分析結果に置き換え
    ...
    // [DUMMY] Predicted Calories専用メッセージ - 本番ではカロリー予測分析結果に置き換え
